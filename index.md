...
```
Before fixing, the for loop first swap the first element with the last element in array. For example, in an array of 3 element {1,2,3}, ```arr[0]= arr[2]``` will make the array look like {3,2,3}. But when the iteration reach the end of the array (```arr[2]```), it makes ```arr[2] = arr[0]```. But here we already changed the value stored in ```arr[0]```, so ```arr[2]=arr[0]``` will make the array look like {3,2,3}.
